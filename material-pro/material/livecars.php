...
if($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if ($checkrow > 0) {
        $sql2 = "SELECT id, carid, brandname, carmake, carmodel, modelyear, carnumber, regstate, demandprice,bidstartprice,currentbid, accidental, cng, color, cengine, fuel, insurance, insurancetype, kms, light, owner, paint, rfc, rusting, suspension, testdriveresult, tyres, windowsshield, workdoneoncar, remarks, cstatus, bidstarttime, bidendtime,bidendsec, livetimeleft, maxbid, minbid FROM carmaster WHERE cstatus = 1";
        ...
        if ($checkrow2 > 0) {
            ...
            while ($row2 = mysqli_fetch_array($run_query2)) {
			   ...
			   $datas[] = array(
					...
					'owner'=>$row2['owner'],
					'kms'=>$row2['kms'],
					'bidstartprice'=>$row2['bidstartprice'],
					'bidendsec'=>$row2['bidendsec'],
